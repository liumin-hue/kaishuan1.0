  ...
  <div class="kaishuan">
    ...
    <div class="time">
      <div class="time_input">
        <button type="button" @click="open('picker1')" placeholder="2019年4月2日" class="time-btn">{{timeStart |
          formatDate()}}
        </button>
        <div class="timetext">至</div>
        <button @click="openpicker('picker2')" placeholder="2019年4月3日" class="time-btn">{{timeEnd |
          formatDate()}}
        </button>
      </div>
    </div>
            ...
            <li v-for="(item,index) in datas" :key="index"  @click="reactConfirm(index,$event)"
              ...
              <div class="model_bottom">
                <div>
                  ...
                  <div><span class="texttitle">开栓时间：</span>{{item.BoltOptTime}}</div>
                </div>
        ...
        methods: {
            ...
            loadFrist() {
                ...
                this.$http.get('http://222.139.181.213:14000/ChargeSystem/Charge/GetCusBoltSet', {
                    params: {
                        //0为未开栓，1为开栓任务列表已下达指令，3为yikaishuan
                        // BoltStatus: 1,  应该值为  1
                        BoltStatus: 3,
                        ...
                        //选择得时间
                        OperateBegTime:this.timeStart ,  //this.timeStart
                        OperateEndTime:this.timeEnd ,  //this.timeEnd
                        //楼号
                    ...
                    .then(response => {
                        response.data.Data.forEach(function (item) {
                            item.BoltOptTime = item.BoltOptTime.substr(0,10)+' '+item.BoltOptTime.substr(11,8)
                        })
                        this.current = 0
